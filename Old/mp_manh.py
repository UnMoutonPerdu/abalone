...
class MyPlayer(PlayerAbalone):
    ...
    def heuristic(self, current_state: GameState):
        ...
        for elem in grid:
            if grid[elem].get_type() == 'W':
                sum_dist_W += self.manhattan_dist(elem, coord_R)
                variance_W += self.manhattan_dist(elem, center_mass_W)**2
            elif grid[elem].get_type() == 'B':
                sum_dist_B += self.manhattan_dist(elem, coord_R)
                variance_B += self.manhattan_dist(elem, center_mass_B)**2
